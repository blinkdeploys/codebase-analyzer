...
README_FEATURE_TEMPLATE = f"""# [TITLE]
...
PROMPT_TEMPLATE = """Analyze this codebase and provide a comprehensive breakdown.
...
COMMIT_PLAN_PROMPT_TEMPLATE = f"""Given this project file summary, divide it into a sequence of logical commits.
...
class CodebaseAnalyzer:
    ...
    # This method handles all code analysis in one go
    def analyze_with_ai(self, codebase):
        ...
        prompt = PROMPT_TEMPLATE.replace("[CODEBASE_SUMMARY]", codebase_summary)
    ...
    def create_git_repo(self, analysis):
        ...
        readme_content = README_FEATURE_TEMPLATE.replace("[TITLE]", analysis.get('description', 'Reconstructed Project'))
                                                .replace("[FEATURES]", chr(10).join(['- ' + f['name'] + ': ' + f['description'] for f in analysis['features']]))
                                                .replace("[TECH_STACK]", ', '.join(analysis.get('tech_stack', [])))
                                                .replace("[BRANCH_STRUCTURE]", chr(10).join(['- `' + f['name'] + '`' for f in analysis['features']]))
    ...
    def run(self):
        ...
        try:
            ...
            analysis = self.analyze_with_ai(codebase)

            # create fresh git repo
            self.create_git_repo(analysis)

            # create all feature branches
            features_completed = self.create_feature_branches(analysis, codebase)

            # merge the features
            self.merge_features(features_completed)
            # enqueue jobs
            job_data = dict(job_id=self.job_id,
                            status="completed",
                            result=dict(output_path=str(self.output_path),
